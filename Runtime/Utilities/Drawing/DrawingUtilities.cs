namespace droid.Runtime.Utilities.Drawing {
  /// <summary>
  /// </summary>
  public static class NeodroidDrawingUtilities {
    /// <summary>
    /// </summary>
    ...
    public static void ForGizmo(UnityEngine.Vector3 pos,
                                UnityEngine.Vector3 direction,
                                UnityEngine.Color color,
                                float arrow_head_length = 0.25f,
                                float arrow_head_angle = 20.0f) {
      UnityEngine.Gizmos.DrawRay(from : pos, direction : direction);
    ...
    public static void ForDebug(UnityEngine.Vector3 pos,
                                UnityEngine.Vector3 direction,
                                UnityEngine.Color color,
                                float arrow_head_length = 0.25f,
                                float arrow_head_angle = 20.0f,
      ...
      if (ray_duration > 0) {
        UnityEngine.Debug.DrawRay(start : pos,
                                  dir : direction,
                                  color : color,
                                  duration : ray_duration);
      } else {
        UnityEngine.Debug.DrawRay(start : pos, dir : direction, color : color);
      }
    ...
    static void NeodroidDrawingUtilitiesEnd(bool gizmos,
                                            ...
                                            float arrow_head_angle = 20.0f,
                                            float ray_duration = 0f) {
      var right = UnityEngine.Quaternion.LookRotation(forward : direction)
                  * UnityEngine.Quaternion.Euler(x : arrow_head_angle, 0, 0)
                  * UnityEngine.Vector3.back;
      var left = UnityEngine.Quaternion.LookRotation(forward : direction)
                 * UnityEngine.Quaternion.Euler(x : -arrow_head_angle, 0, 0)
                 * UnityEngine.Vector3.back;
      var up = UnityEngine.Quaternion.LookRotation(forward : direction)
               * UnityEngine.Quaternion.Euler(0, y : arrow_head_angle, 0)
               * UnityEngine.Vector3.back;
      var down = UnityEngine.Quaternion.LookRotation(forward : direction)
                 * UnityEngine.Quaternion.Euler(0, y : -arrow_head_angle, 0)
                 * UnityEngine.Vector3.back;
      if (gizmos) {
        UnityEngine.Gizmos.color = color;
        UnityEngine.Gizmos.DrawRay(from : pos + direction, direction : right * arrow_head_length);
        UnityEngine.Gizmos.DrawRay(from : pos + direction, direction : left * arrow_head_length);
        UnityEngine.Gizmos.DrawRay(from : pos + direction, direction : up * arrow_head_length);
        UnityEngine.Gizmos.DrawRay(from : pos + direction, direction : down * arrow_head_length);
      } else {
        if (ray_duration > 0) {
          UnityEngine.Debug.DrawRay(start : pos + direction,
          ...
          UnityEngine.Debug.DrawRay(start : pos + direction, dir : right * arrow_head_length, color : color);
          UnityEngine.Debug.DrawRay(start : pos + direction, dir : left * arrow_head_length, color : color);
          UnityEngine.Debug.DrawRay(start : pos + direction, dir : up * arrow_head_length, color : color);
          UnityEngine.Debug.DrawRay(start : pos + direction, dir : down * arrow_head_length, color : color);
        }
      }
 ...
 */
    ...
    /// <param name="rb"></param>
    /// <returns></returns>
    public static float KineticEnergy(UnityEngine.Rigidbody rb) {
      return 0.5f
             * rb.mass
             * UnityEngine.Mathf.Pow(f : rb.velocity.magnitude,
    ...
    public static void DrawLine(UnityEngine.Vector3 p1, UnityEngine.Vector3 p2, float width) {
      var count = UnityEngine.Mathf.CeilToInt(f : width); // how many lines are needed.
      if (count == 1) {
        UnityEngine.Gizmos.DrawLine(from : p1, to : p2);
      } else {
        var c = UnityEngine.Camera.current;
        if (c == null) {
        ...
        var n = UnityEngine.Vector3.Cross(lhs : v1, rhs : v2); // normal vector
        for (var i = 0; i < count; i++) {
          //Vector3 o = n * width ((float)i / (count - 1) - 0.5f);
          var o = width * ((float)i / (count - 1) - 0.5f) * n;
          UnityEngine.Gizmos.DrawLine(from : p1 + o, to : p2 + o);
        }
      }
    }
    ...
    public static UnityEngine.Texture2D RenderTextureImage(UnityEngine.Camera camera) {
      ...
      var texture = new UnityEngine.Texture2D(width : target_texture.width, height : target_texture.height);
      texture.ReadPixels(source : new UnityEngine.Rect(0,
                                                       0,
    ...
    public static string
        ColorArrayToString(System.Collections.Generic.IEnumerable<UnityEngine.Color> colors) {
      var s = "";
